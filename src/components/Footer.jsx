            ...
            <div key={footerLink.title} className='flex flex-col ss:my-0 my-4 min-w-[150px]'>
              ...
              <ul className='list-none mt-4'>
                {footerLink.links.map((link, index) => (
                  <li key={link.name} className={`font-poppins font-normal text-[16px] leading-[24px] text-dimWhite hover:text-secondary cursor-pointer ${index !== footerLink.lenth -1 ?'mb-4' : 'mb-0' }`}>{link.name}</li>
                ))}
              </ul>
            </div>
          ))}
      ...
      <div className='w-full flex justify-between items-center md:flex-row flex-col pt-6 border-t-[1px] border-t-[#3F3E45]'>
        ...
        <div className='flex flex-row md:mt-0 mt-6'>
          {socialMedia.map((socail, index) => (
            <img key={socail.id} src={socail.icon} alt={socail.id} className={`w-[21px] h-[21px] object-contain cursor-pointer ${index !== socialMedia.length-1 ? 'mr-6' : 'mr-0'}`} />
          ))}
        </div>
      </div>
    </section>
